...
int StudentWorld::doThings(){
	for (auto& actors : actor_vec) { // Iterates through entire vector of actor objects 
		...
		if (actors && actors->isAlive() && actors->type != Actor::ActorType::player) // If it is valid and exists, have the actor do something.
			actors->doSomething(); 
		if (player && player->isAlive()) // If the player is still alive, have them do something.
			player->doSomething(); 
		else
